...
def handle_retrieve_garmin_workouts_tool(tool_use, garmin_client: Garmin):
    ...
    try:
        ...
        if not workouts:
            ...
            workout_lines = [
                f"Retrieved {len(workouts)} workouts from Garmin Connect:\n"
            ]
        ...
        return {
            ...
            "content": [{"type": "text", "text": workout_text}],
        }
...
def handle_retrieve_garmin_workout_details_tool(tool_use, garmin_client: Garmin):
    ...
    if not workout_id:
        return {
            "tool_use_id": tool_use.id,
            "content": [{"type": "text", "text": "Error: workout_id is required"}],
        }
    ...
    try:
        ...
        return {
            "tool_use_id": tool_use.id,
            "content": [
                {"type": "text", "text": str(workout_detail) or "No workouts found."}
            ],
        }
