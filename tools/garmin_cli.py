...
def print_methods(garmin_client):
    ...
    methods = [method for method in dir(garmin_client) 
            ...
            while True:
                try:
                    ...
                    if not hasattr(garmin_client, method_name):
                        print(f"Error: Method '{method_name}' not found. Type 'help' to see available methods.")
                        continue
                    ...
                    print(f"Calling {method_name}({', '.join(map(str, args))}{', ' if args and kwargs else ''}{', '.join(f'{k}={v}' for k, v in kwargs.items())})...")
